...
HRESULT CLevel_Stage4::Ready_Layer_Potal(const _wstring& strLayerTag)
{
...
HRESULT CLevel_Stage4::Ready_Observer()
{
	// 옵저버 매니저에 Observer_ClearTrigger Key값을 가진 CClearTriggerObserver 생성
	if(FAILED(m_pGameInstance->Add_Observer(ENUM_CLASS(LEVEL::LEVEL_STAGE4), TEXT("Observer_ClearTrigger"),
...
HRESULT CLevel_Stage4::Ready_Layer_ColliderMap(const _wstring& strLayerTag)
{
	...
	for (_uint i = 0; i < Stage_ColliderCount(ENUM_CLASS(LEVEL::LEVEL_STAGE4)); ++i)
	{
		auto Pair = MapCollider_Builder(ENUM_CLASS(LEVEL::LEVEL_STAGE4), i);
