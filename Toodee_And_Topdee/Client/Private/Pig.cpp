...
void CPig::Priority_Update(_float fTimeDelta)
{
	...
	if (m_pGameInstance->Get_CurrentDimension() == DIMENSION::CHANGE)
	{
		...
		if (fDot < 0) // TOPDEE로 전환시 혹시라도 TOODEE에서 방향이 뒤집혔다면 돌리는 작업
		{
...
_bool CPig::Check_Gravity(_float fTimeDelta) // Dimension이 TooDee일 경우만 들어오는 함수
{
	...
	for (auto& Other : *findAll)
	{
			...
			strOtherName.find(TEXT("Wall")) != string::npos)
		{
			...
			if (OtherPos.z < vMyPos.z && vDelta.z > m_vColliderScale.z * 0.5f)// 충돌객체가 객체보다 밑에있다 (객체기준 8방향중 하단 3방향)
			{
	...
	if (m_pColliderCom->GetOverlapAll(findAll))
	{
		...
		if (1 == findAll->size()) // 충돌객체가 1개이며 바닥 객체이다 (바닥객체와 1:1충돌시에만 중력 off기 때문에 다른상황은 없음)
		{
			for (auto& Other : *findAll)
			{
				...
				if (fDeltaX > 0 && fDeltaX > (vOtherRadius.x)) // 양수, 객체보다 충돌체가 좌측
				{
					m_pTransformCom->TurnToRadian(_float3(0.0f, 0.0f, 1.0f), D3DXToRadian(180.f));
					m_pTransformCom->Go_Right(fTimeDelta * 2.f);
					return;
				}
				else if (fDeltaX < 0 && fDeltaX < -vOtherRadius.x)  // 음수, 객체보다 충돌체가 우측
				{
					m_pTransformCom->TurnToRadian(_float3(0.0f, 0.0f, 1.0f), D3DXToRadian(180.f));
					m_pTransformCom->Go_Right(fTimeDelta * 2.f);
					return;
		...
		else // 충돌체가 2개 이상
		{
			for (auto& Other : *findAll)
			{
				...
				_float fOtherMaxZ = vOtherPos.z + vOtherRadius.z; // 충돌 객체의 중심 z값 + z값에 대한 반지름 (충돌객체의 높이 판단) 벽이 아니라면 포지션값보다 높을수가 없음
...
HRESULT CPig::Render_Shadow()
{
	Compute_AttributeShadow();

	_float3 vGroundPoint = _float3(0.f, -0.49f, 0.f);
	...
	matWorld._11 = 1.7f;    matWorld._12 = matWorld._13 = 0.f;
	matWorld._22 = 1.f;     matWorld._21 = matWorld._23 = 0.f;
	matWorld._33 = 1.7f;    matWorld._31 = matWorld._32 = 0.f;
	matWorld._43 -= 0.5f;
...
void CPig::Compute_AttributeShadow()
{
