...
void CLevel_FinalBoss02::Update(_float fTimeDelta)
{
	...
	{
		if (FAILED(m_pGameInstance->Open_Level(static_cast<_uint>(LEVEL::LEVEL_LOADING), CLevel_Loading::Create(m_pGraphic_Device, LEVEL::LEVEL_FINALBOSS3))))
			return;
	}
...
void CLevel_FinalBoss02::CreateSemiclon(_float fTimeDelta)
{
	...
	if (m_fDelayTime >= 1.f)
	{
		...
		m_pGameInstance->Add_GameObject_ToLayer(ENUM_CLASS(LEVEL::LEVEL_FINALBOSS2), TEXT("Layer_Semiclon"),
			ENUM_CLASS(LEVEL::LEVEL_FINALBOSS2), TEXT("Prototype_GameObject_Semiclon"), &vPos);
		m_fDelayTime = 0;
		++Count;
